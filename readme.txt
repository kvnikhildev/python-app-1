...
install java and jenkin docker using ansible playbook -- file provided
  ansible-playbook -i inventry install.yml
check the version jenkin java and port using adhock cmd
...
open port for jenkin in ec2 instance using aws-cli 
  aws ec2 describe-instances --query "Reservations[].Instances[].[InstanceId,SecurityGroups[]]"   # to get security group
  aws ec2 authorize-security-group-ingress --group-id sg-0d28b5446864a4b43 --protocol tcp --port 8080 --cidr 0.0.0.0/0   # for jenkin
  aws ec2 authorize-security-group-ingress --group-id sg-0d28b5446864a4b43 --protocol tcp --port  5000 --cidr 0.0.0.0/0   # for python app
...
add plugin 
 docker pipeline
 give access to the jenkin user on ec2 using ansible adhoc comments
...
open jenkin 
create pipeline
 add jenkin file use scm naavigate the git repositry
